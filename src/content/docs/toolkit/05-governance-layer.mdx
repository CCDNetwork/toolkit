...
The Governance layer can be the most difficult to understand - but it's the most important part of any interoperability initiative, because it asks the question - who is allowed to make decisions about the data which we collect and share?

Although many humanitarian organisations have internal policies which govern their use of data - including data protection policies - there is a lack of shared governance mechanisms. That's a real problem when you are working together in a cluster or consortium, and if we don't address it we will find somebody else making decisions on our behalf, with little or no accountability.

As the humanitarian sector becomes increasingly data-driven, it is therefore important to develop new governance mechanisms. At present most data governance is purely hierarchical - a large centralised agency gathers the data from partners and decides what to do with it. This approach is sometimes appropriate, but often has many weaknesses.

The biggest weaknesses are that it lacks any oversight of the data by the agencies that collect and share that data - and lacks transparency to the communities that provide that data. We want to particularly emphasise the latter point. We propose that the humanitarian community in general needs to take the data rights of disaster-affected individuals and households more seriously.

In order to help humanitarian coordination groups to address this lack of oversight and transparency, we developed an alternative collective approach to the existing model. This data stewardship model is embodied in a Governance Framework which we have developed - and, like all the tools in this Toolkit - which can be adapted as necessary.
